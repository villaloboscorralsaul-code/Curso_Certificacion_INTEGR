...
export type Lab = {
  code: string; icon: string; title: string; question: string; simple: string; scenario: string; formula: string; use: string;
  fields: { key: string; label: string; unit: string; help: string }[];
  compute: (values: Record<string, string>) => { value: number; unit: string; work: string };
  interpret: (value: number) => string;
  challenge: string; expected: number; tolerance: number; challengeUnit: string; hint: string;
};
type Props = {
  completed: boolean; onComplete: () => void;
  labs: Record<string, Lab>; initialValues: Record<string, string>; storageKey: string;
  eyebrow: string; heading: string; subheading: string; menuLabel: string;
};
...
export const day1Labs: Record<string, Lab> = {
  ohm: { code: "LAB 01", icon: "V÷Ω", title: "Ley de Ohm", question: "¿Cuánta corriente circulará?", simple: "El voltaje empuja la corriente y la resistencia limita su paso.", scenario: "Un calentador industrial se conecta a una fuente de 230 V. Su resistencia es de 28.75 Ω. Antes de energizarlo, necesitamos estimar la corriente.", formula: "I = V ÷ R", use: "Anticipar corriente y compararla con la placa, el conductor y la protección.", fields: [{ key: "voltage", label: "Voltaje disponible", unit: "V", help: "El empuje eléctrico de la fuente." }, { key: "resistance", label: "Resistencia de la carga", unit: "Ω", help: "La oposición de la carga al paso de corriente." }], compute: (v) => { const n = (k: string) => Number(v[k]) || 0; const value = n("resistance") ? n("voltage") / n("resistance") : 0; return { value, unit: "A", work: `${n("voltage")} ÷ ${n("resistance")}` }; }, interpret: (value) => `La carga permitiría aproximadamente ${value.toFixed(2)} A. Compáralos con la corriente nominal, el conductor y la protección. El cálculo anticipa una condición; no reemplaza una medición segura.`, challenge: "Si conectas una resistencia de 20 Ω a 120 V, ¿cuánta corriente circulará?", expected: 6, tolerance: .05, challengeUnit: "A", hint: "Divide 120 entre 20." },
  potencia: { code: "LAB 02", icon: "kW", title: "Potencia activa", question: "¿Cuánta energía se convierte en trabajo?", simple: "La potencia activa es la parte de la energía eléctrica que realmente produce trabajo.", scenario: "Una carga opera a 230 V, consume 10 A y tiene factor de potencia 0.86. Queremos estimar la potencia activa que utiliza.", formula: "P = V × I × FP ÷ 1000", use: "Estimar consumo y comparar condiciones de operación de una carga.", fields: [{ key: "voltage", label: "Voltaje medido", unit: "V", help: "Tensión presente durante la medición." }, { key: "current", label: "Corriente medida", unit: "A", help: "Carga eléctrica que circula." }, { key: "pf", label: "Factor de potencia", unit: "FP", help: "Proporción entre potencia útil y demanda total." }], compute: (v) => { const n = (k: string) => Number(v[k]) || 0; const value = n("voltage") * n("current") * n("pf") / 1000; return { value, unit: "kW", work: `${n("voltage")} × ${n("current")} × ${n("pf")} ÷ 1000` }; }, interpret: (value) => `La carga convierte aproximadamente ${value.toFixed(2)} kW en potencia activa con estos datos. En sistemas trifásicos la relación cambia y debe considerarse √3.`, challenge: "Una carga opera a 200 V, 5 A y FP 0.80. ¿Cuál es su potencia activa?", expected: .8, tolerance: .02, challengeUnit: "kW", hint: "Multiplica 200 × 5 × 0.80 y divide entre 1000." },
  eficiencia: { code: "LAB 03", icon: "η", title: "Eficiencia", question: "¿Qué porcentaje llega al trabajo útil?", simple: "La máquina recibe energía, aprovecha una parte y pierde otra como calor, fricción o ruido.", scenario: "Un motor recibe 18.5 kW y entrega 15 kW en su eje. Queremos saber qué porcentaje de la entrada se convierte en salida útil.", formula: "η = P salida ÷ P entrada × 100", use: "Comparar equipos y detectar aumento de pérdidas o deterioro.", fields: [{ key: "input", label: "Potencia de entrada", unit: "kW", help: "Todo lo que el equipo recibe." }, { key: "output", label: "Potencia útil de salida", unit: "kW", help: "La parte transformada en trabajo útil." }], compute: (v) => { const n = (k: string) => Number(v[k]) || 0; const value = n("input") ? n("output") / n("input") * 100 : 0; return { value, unit: "%", work: `${n("output")} ÷ ${n("input")} × 100` }; }, interpret: (value) => `De cada 100 unidades que entran, ${value.toFixed(1)} se convierten en trabajo útil. El resto aparece principalmente como calor, fricción, ventilación o ruido.`, challenge: "Una máquina recibe 10 kW y entrega 8.5 kW. ¿Cuál es su eficiencia?", expected: 85, tolerance: .1, challengeUnit: "%", hint: "Divide 8.5 entre 10 y multiplica por 100." },
  perdidas: { code: "LAB 04", icon: "I²R", title: "Pérdidas en cable", question: "¿Cuánta potencia termina como calor?", simple: "La corriente y la resistencia del cable producen calor. La corriente influye al cuadrado.", scenario: "Un circuito conduce 10 A por 120 m. El conductor presenta 0.00115 Ω por metro. Calcularemos la pérdida considerando ida y vuelta.", formula: "P pérdida = I² × R/m × distancia × 2 ÷ 1000", use: "Analizar calentamiento, longitud y selección del conductor.", fields: [{ key: "current", label: "Corriente", unit: "A", help: "Valor que circula por el conductor." }, { key: "distance", label: "Distancia de ida", unit: "m", help: "La fórmula considera ida y regreso." }, { key: "cableR", label: "Resistencia por metro", unit: "Ω/m", help: "Dato propio del material y calibre." }], compute: (v) => { const n = (k: string) => Number(v[k]) || 0; const value = n("current") ** 2 * n("cableR") * n("distance") * 2 / 1000; return { value, unit: "kW", work: `${n("current")}² × ${n("cableR")} × ${n("distance")} × 2 ÷ 1000` }; }, interpret: (value) => `El cable disipa aproximadamente ${value.toFixed(3)} kW como calor. Si la corriente aumenta, la pérdida crece rápidamente porque la corriente está elevada al cuadrado.`, challenge: "Con 10 A, 100 m y 0.001 Ω/m, ¿cuántos kW se pierden?", expected: .02, tolerance: .002, challengeUnit: "kW", hint: "Calcula 10² × 0.001 × 100 × 2 ÷ 1000." },
};

export const day1InitialValues: Record<string, string> = { voltage: "230", resistance: "28.75", current: "10", pf: "0.86", input: "18.5", output: "15", distance: "120", cableR: "0.00115" };

export default function PracticeExperience({ completed, onComplete, labs, initialValues, storageKey, eyebrow, heading, subheading, menuLabel }: Props) {
  const labIds = Object.keys(labs);
  const [labId, setLabId] = useState(labIds[0]);
  const [phase, setPhase] = useState<Phase>("observar");
  const [values, setValues] = useState(initialValues);
  const [answer, setAnswer] = useState("");
  const [checked, setChecked] = useState(false);
  const [finishedLabs, setFinishedLabs] = useState<string[]>([]);
  const lab = labs[labId];
  const phaseIndex = phases.findIndex((item) => item.id === phase);

  useEffect(() => { const saved = localStorage.getItem(storageKey); if (saved) setFinishedLabs(JSON.parse(saved)); }, [storageKey]);

  const result = useMemo(() => lab.compute(values), [lab, values]);

  function selectLab(id: string) { setLabId(id); setPhase("observar"); setAnswer(""); setChecked(false); }
  function checkAnswer() {
    setChecked(true);
    if (Math.abs(Number(answer) - lab.expected) <= lab.tolerance && !finishedLabs.includes(labId)) {
      const next = [...finishedLabs, labId]; setFinishedLabs(next); localStorage.setItem(storageKey, JSON.stringify(next));
    }
  }
  function speak() { if (!("speechSynthesis" in window)) return; speechSynthesis.cancel(); const text = phase === "observar" ? `${lab.title}. ${lab.scenario}. ${lab.simple}` : phase === "calcular" ? `${lab.formula}. El resultado actual es ${result.value.toFixed(result.unit === "%" ? 1 : 2)} ${result.unit}` : phase === "interpretar" ? lab.interpret(result.value) : `${lab.challenge}. Pista: ${lab.hint}`; const voice = new SpeechSynthesisUtterance(text); voice.lang = "es-MX"; voice.rate = .9; speechSynthesis.speak(voice); }
  const correct = checked && Math.abs(Number(answer) - lab.expected) <= lab.tolerance;

  return <section className="practice-experience">
    <header className="practice-experience-head"><div><span>{eyebrow}</span><h1>{heading}</h1><p>{subheading}</p></div><div className="practice-view-tools"><button onClick={speak}>▶ Escuchar</button></div></header>
    ...
    <div className="practice-layout">
      <aside className="lab-menu"><span>{menuLabel}</span>{labIds.map((id) => <button key={id} onClick={() => selectLab(id)} className={`${id === labId ? "active" : ""} ${finishedLabs.includes(id) ? "done" : ""}`}><i>{labs[id].icon}</i><div><small>{labs[id].code}</small><b>{labs[id].title}</b></div><em>{finishedLabs.includes(id) ? "✓" : "→"}</em></button>)}<div className="lab-menu-progress"><b>{finishedLabs.length}/{labIds.length}</b><span>laboratorios comprobados</span></div></aside>
      <main className="practice-main">
        ...
        <footer className="practice-presenter-controls">{phaseIndex < phases.length - 1 ? <button className="next" onClick={() => setPhase(phases[phaseIndex + 1].id)}>Siguiente → {phases[phaseIndex + 1].verb}</button> : <button className="next" disabled={!finishedLabs.includes(labId)} onClick={() => finishedLabs.length === labIds.length ? onComplete() : selectLab(labIds.find((id) => !finishedLabs.includes(id)) ?? labIds[0])}>{!finishedLabs.includes(labId) ? "Resuelve el reto" : finishedLabs.length === labIds.length ? "Completar práctica ✓" : "Siguiente laboratorio →"}</button>}</footer>
      </main>
    </div>
    <div className="practice-final-bar"><span className={completed ? "done" : ""}>{completed ? "✓ Práctica completada" : `Completa los ${labIds.length} retos para cerrar la práctica`}</span></div>
  </section>;
}
