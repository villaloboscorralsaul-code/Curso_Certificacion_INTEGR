...
export default function CourseAdmin() {
  ...
  return <main className="admin-shell">
    <aside className={`sidebar ${menuOpen ? "open" : ""}`}><button className="close-menu" onClick={() => setMenuOpen(false)} aria-label="Cerrar menú">×</button>
      ...
      <div className="side-label course-label">PLAN DE 5 DÍAS <em>1 / 5</em></div><div className="day-list">{days.map((day) => <button key={day.day} disabled={!day.active && day.day !== 2} className={`day-button ${day.active || day.day === 2 ? "current" : "locked"}`} onClick={() => day.day === 2 ? go("dia2") : day.active && go("inicio")}><span className="day-number">{String(day.day).padStart(2,"0")}</span><span><b>Día {day.day}</b><small>{day.title}</small></span><i>{day.active ? "●" : "◌"}</i></button>)}</div>
