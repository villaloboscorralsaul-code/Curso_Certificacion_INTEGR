...
export default function CourseAdmin() {
  ...
  return <main className="admin-shell">
    <aside className={`sidebar ${menuOpen ? "open" : ""}`}><button className="close-menu" onClick={() => setMenuOpen(false)} aria-label="Cerrar menú">×</button>
      ...
      <div className="side-label course-label">PLAN DE 5 DÍAS <em>2 / 5</em></div><div className="day-list">{days.map((day) => { const available = day.active || day.day === 2; const selected = day.day === 2 ? view === "dia2" : day.day === 1 && view !== "dia2"; return <button key={day.day} disabled={!available} className={`day-button ${selected ? "current" : available ? "available" : "locked"}`} onClick={() => day.day === 2 ? go("dia2") : day.active && go("inicio")}><span className="day-number">{String(day.day).padStart(2,"0")}</span><span><b>Día {day.day}</b><small>{day.title}</small></span><i>{available ? "●" : "◌"}</i></button> })}</div>
    ...
    <section className="workspace"><header className="topbar"><button className="menu-toggle" onClick={() => setMenuOpen(true)} aria-label="Abrir menú">☰</button><div className="crumbs"><span>Habilidades electromecánicas</span><b>/</b><strong>{view === "inicio" ? "Día 1" : view === "dia2" ? "Día 2" : sections.find((item) => item.id === view)?.label}</strong></div><div className="top-actions"><span className="live-dot">Publicado</span><button className="avatar" aria-label="Perfil del administrador">SG</button></div></header>
      ...
      {view === "dia2" && <DayTwoExperience onBack={() => go("inicio")} onProgressChange={setDayTwoProgress} />}
